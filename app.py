...
with tab5:
    ...
    results.append({
        ...
        fig = go.Figure()
        fig.add_trace(go.Scatter(x=df.index, y=df['Close'], mode='lines', name='ราคา', line=dict(color='blue', width=1)))
        fig.add_trace(go.Scatter(x=df.index, y=df['EMA200'], mode='lines', name='EMA200', line=dict(color='red', width=1.5, dash='dash')))
        ...
        fig.update_layout(title=f"ราคา {analyzer.thai_stocks[selected_code]}", height=500)
        st.plotly_chart(fig, use_container_width=True)
